#!/usr/bin/env python3
"""
Reference library downloader.

Reads references-manifest.json and populates each entry's image. Each entry
can specify either:
  - "url": explicit image URL to download (preferred for stability)
  - "search": eBay search query — script finds a matching listing, scrapes the
              s-l1600 image URL, and downloads it (true automation)
  - both: tries url first, falls back to search

Idempotent: by default skips files that already exist. Pass --mode refresh-all
to re-download everything.

Manifest format:
  {
    "<repo-path>.jpg": {
      "search": "Nyara Sabally 2025 Prizm WNBA 117",
      "url": "https://i.ebayimg.com/...s-l1600.jpg",
      "alt_urls": ["https://...", "..."],
      "_note": "..."
    },
    ...
...
def search_ebay_for_image(query):
    """Search eBay listings for `query`, return the s-l1600 stock photo URL of the first card-shaped result."""
    ...
    if not item_match:
        print(f"   ✗ No listing found for: {query}")
        return None
    ...
    print(f"   ✗ No image extracted from: {listing_url}")
    return None
